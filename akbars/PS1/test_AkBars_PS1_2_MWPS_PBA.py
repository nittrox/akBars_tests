...
class GeneratorUser(User):
    def __init__(self, mcc_list, z_list, currency, parametr_option):
        for k in range(0, int(len(CodeList) / 2)):
            ...
            # if parametr_option == mili:
            #     url_4 = f"{url_list[3]}?cardMnemocode={cardMnemocode}&productOptionId=10801&profileId={profileId} "
            #     payload_4 = {}
            #     headers_2 = {
            #         'accept': '*/*'
            #     }
            #     response_3 = requests.request("POST", url_4, headers=headers_2, data=payload_4)
            ...
            for p in range(0, 6):
                ...
                # if parametr_option == mili:
                #     cursor.execute(
                #         "UPDATE  apps.user_options  SET option_id=(%s)"
                #         " WHERE profile_id  = (%s)",
                #         ('11801', profileId,))
                #     conn.commit()
                #
                #     cursor.execute(
                #         "UPDATE  apps.user_options  SET product_option_id =(%s)"
                #         " WHERE profile_id  = (%s)",
                #         ('74901', profileId,))
                #     conn.commit()
...
def test_CreateUser():
    g = GeneratorUser(mcc_list=["5811", "5812", "5813", "5814", "5137", "7999"],
                      z_list=[40000, 598698, 2345345, 234562323, 34523452, 563212], currency=810, parametr_option="cashback" )
...
def test_checkClickHouseSum_1():
    with allure.step(f"Кэшбэк-рубли Максимальная ежемесячная сумма начислений = 2000 кэшбэк-руб"):
        g = GeneratorUser(mcc_list=["4111", "5811", "9950", "6050", "5811", "5811"],
                          z_list=[10000000, 10000000, 10000000, 10000, 10000000, 10000000], currency=810,
                          parametr_option="cashback")
        time.sleep(conts_time)
...
def test_checkClickHouseSum_2():
    with allure.step(f"Универсальные мили Проверка вознаграждение по операциям = 1%"):
        g = GeneratorUser(mcc_list=["4111", "5811", "9950", "6050", "5811", "5811"],
                          z_list=[20000, 20000, 20000, 20000, 20000, 20000], currency=810, parametr_option="mili")
        time.sleep(conts_time)
...
def test_checkClickHouseSum_3():
    with allure.step(f"Универсальные мили Максимальная ежемесячная сумма начислений = 5000 миль"):
        g = GeneratorUser(mcc_list=["4111", "5811", "9950", "6050", "5811", "5811"],
                          z_list=[100000000, 100000000, 100000000, 100000, 100000000, 100000000], currency=810,
...
def test_checkClickHouseSum_4():
    with allure.step(f"Бонусы партнерских программ (кроме Пятерочка и Перекресток) S7 - Проверка вознаграждение по "
                     f"операциям = 1%"):
        g = GeneratorUser(mcc_list=["4111", "5811", "9950", "6050", "5811", "5811"],
                          z_list=[20000, 20000, 20000, 20000, 20000, 20000], currency=810, parametr_option="s7")
        time.sleep(conts_time)
...
def test_checkClickHouseSum_5():
    with allure.step(f"Бонусы партнерских программ (кроме Пятерочка и Перекресток) Максимальная ежемесячная сумма "
                     f"начислений = 2000 бонусов "):
        g = GeneratorUser(mcc_list=["4111", "5811", "9950", "6050", "5811", "5811"],
                          z_list=[100000000, 100000000, 100000000, 100000, 100000000, 100000000], currency=810,
                          parametr_option="s7")
        time.sleep(conts_time)
...
def test_checkClickHouseSum_6():
    with allure.step(f"Бонусы партнерских программ (Пятерочка и Перекресток)- Проверка вознаграждение по "
                     f"операциям = 10%"):
        g = GeneratorUser(mcc_list=["4111", "5811", "9950", "6050", "5811", "5811"],
                          z_list=[20000, 20000, 20000, 20000, 20000, 20000], currency=810,
...
def test_checkClickHouseSum_7():
    with allure.step(f"Бонусы партнерских программ (Пятерочка и Перекресток) Максимальная ежемесячная сумма "
                     f"начислений = 20000 бонусов "):
        g = GeneratorUser(mcc_list=["4111", "5811", "9950", "6050", "5811", "5811"],
                          z_list=[100000000, 100000000, 100000000, 100000, 100000000, 100000000], currency=810,
                          parametr_option="perekrestok")
        time.sleep(conts_time)
